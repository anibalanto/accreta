...
**`track` en un repo que todavía no tiene `.bilink/` deja una ref en la que `accept` no escribe nunca.** `track` la crea con el árbol vacío, y después cada `accept` absorbe la rama, dice que no está absorbida y aborta, y deja un commit `absorb` más en cada intento. Mientras no se arregle, un repo nuevo sigue el corte 005: primero `.bilink/` commiteado en la rama, después un commit que lo saca del índice, y recién ahí `init` y `track`.
