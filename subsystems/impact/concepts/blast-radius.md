...
2. **Propagación reactiva**: cada nodo alterado cambia el contenido de su archivo
   `.bilink`. Esto hace que el hash del `.bilink` vecino ya no coincida con ninguna entrada en `accepted.N` del nodo adyacente, propagando `CHAIN_DIRTY` hacia los extremos.
...
impact calcula el blast radius consultando todos los `.bilink` files del proyecto para encontrar qué cadenas referencian `F`, y luego recorriendo cada cadena para identificar todos los nodos que se verán afectados.
...
El mecanismo de propagación es el descripto en bilink-format: `state.N` es parte del archivo `.bilink`, por lo que cambiar el estado de un nodo cambia el hash que el nodo adyacente tiene almacenado, desencadenando `CHAIN_DIRTY` en el próximo check.

impact no replica ese mecanismo — lo lee. El blast radius que calcula impact es una proyección de lo que bilinker detectaría si se corriese `check` en todas las capas afectadas.
