...
**Estado:** borrador, con el primer slice ya en ejecución. Salió de una conversación de diseño y de una primera subida a Jira que se frenó antes de crear nada.
...
**Es la misma forma que el sprint que está corriendo.** Está medido del lado de `bilinker`: *"`check <un archivo>` verifica la capa entera — 98 bilinks y 391 queries al proveedor para preguntar por uno"*. Acá es idéntico un piso más arriba, y la respuesta también: **acotar el alcance de la pregunta al alcance del cambio.**
...
**Esto le pasa por encima a la decisión anterior sobre los ids**, que fijaba `<proyecto>-<id>` con base-36 local. `ACC-235` cumple ese propósito mejor —el board está en el id por construcción y no por convención— pero aquella decisión queda para reescribir, no para archivar: la convención de commits y el formato del endpoint `issue` cambian los dos, y la regla de lectura de los commits viejos sigue haciendo falta.
...
> Una versión anterior de este borrador decía que una ventana **sí** podía partir un subárbol, apoyándose en un párrafo de `hierarchy.md` que contradecía a la regla del ancestro. Ese párrafo era el error y ya se corrigió. Lo que queda acá es más simple: el único ancestro que una ventana hereda es la épica.
...
Lo que cuesta, medido: la decisión de declarar *cuál capa* es el board queda sin objeto —si no es una capa no hay nada que declarar—; `stratum pull` deja de traerlo y pasa a ser un `git clone`; bilinker cambia su resolución del endpoint `issue`; y el `.gitignore` de accreta necesita una línea propia, porque hoy `**/.stratum/*/` lo cubría de arriba. Los links relativos de ítems a specs no entran en la cuenta: ya se convirtieron a permalinks absolutos, salvo los 11 que apuntan a `impact`, que no tiene repo publicado.
