...
**Estado:** no implementado. Sale del reclamo de una política de retención, y del costo que la `003` cobró por no tenerla. Para el corte de la `003` **no se aplica**: su backup ya existe y su ventana se cierra sola cuando la restitución termine. Esto es para el próximo.
...
[`commands/migrate.md`](../commands/migrate.md), que es hoy el único lugar donde el backup del corte se nombra: dónde lo escribe, qué dice al terminar, y la regla. `--rollback` restaura desde el directorio suelto, así que mover el hogar lo mueve a él también, o lo saca — es una de las tres preguntas que la política de retención deja abiertas.
