...
Las dos primeras son una respuesta sobre el árbol. La tercera es la falta de una, y colapsarlas hace que el resumen final afirme algo que nadie verificó. Es la misma regla que `concepts/language-servers.md` aplica del otro lado de la frontera —*"un vacío se leería como no hay llamadas"*— y la que `2q` persigue en el reporte de `check`.
