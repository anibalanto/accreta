...
**No decide si una capa debería serlo.** Eso lo dice stratum, y bilinker no lo consulta — `1w` deja abierto si `check` debería avisar al encontrar un `.bilink/` bajo la capa actual que no corresponde a una capa declarada. Este comando arregla el caso; no lo detecta.
