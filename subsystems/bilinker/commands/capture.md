...
**La selección elige un nodo, no un rango.** Se usa para encontrar el ancla estable que la contiene, y después se descarta: el capture es ese nodo entero. Seleccionar media función captura la función. Ver [`concepts/capture.md`](../concepts/capture.md) § "No hay sub-rango".
...
5. Construir la query como el camino del AST desde ese ancestro hasta el nodo target. Cada predicado usa un nombre de captura único (`@n0`, `@n1`, …). El `@target` se coloca en el nodo que representa el fragmento completo.
   ...
   **El nombre entra escapado.** Un predicado es un string adentro de una query, así que `\` y `"` no se pueden escribir tal cual: `\n` adentro de una query es un salto de línea y no dos caracteres, y un `"` cierra el string y hace inválida la query entera. Se escapa en **un solo lugar**, el mismo que usa [`recapture`](recapture.md) al reescribir el nombre — escribir y reescribir el mismo campo con reglas distintas hace que el mismo nombre produzca dos queries según por qué camino se generó, que es cómo se descubrió.
...
Dos campos posibles y ninguno más: son los que entran en el id. `range`, `state` y `resolved_at` no se escriben — el primero es derivado y vive en [la cache](../concepts/cache.md), y los otros dos no existen.

### Una selección parcial captura el nodo entero
...
La selección cubría media línea del párrafo; el capture es el párrafo. **Para capturar algo más chico hace falta una query que lo nombre**, no un recorte sobre una que nombra algo más grande — ver [`concepts/capture.md`](../concepts/capture.md) § "No hay sub-rango".
