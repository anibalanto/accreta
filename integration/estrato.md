...
`accreta/.worklist/` sigue exactamente la convención `.stratum/` de Stratum. **No es una capa interna**: es un repo aparte clonado adentro de `.stratum/`, igual que las capas `impl` de cada subsistema, y accreta la ignora con la misma regla que a ellas — una sola línea, `**/.stratum/*/`, en el `.gitignore` de la raíz.
